const onePage = document.querySelector('.contributor-profile')
...
.then((data) => {
  ...
  const one =(data.data[0])
    ...
    oneContributor += `
    <li><h3>${one.name}</h3></li><br><li>${one.text}`; 
     
    onePage.innerHTML = oneContributor;
...
window.addEventListener('scroll', () => {
  ...
  if(window.scrollY > 200){
    ...
    setTimeout((e) => {
      ...
      newEnhanedBrand.innerHTML = oneContributor;
